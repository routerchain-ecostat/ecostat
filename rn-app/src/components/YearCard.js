...
export default function YearCard({ d, year, onLayout }) {
  const { getYR, yi } = useApp();
  ...
  const improving = dv == null || d.good === "flat" ? null : d.good === "up" ? dv > 0 : dv < 0;
  ...
  const useBar = d.chart === "bar" || d.id === "nfp" || d.id === "gdp";
  ...
  return (
    <View style={styles.card} onLayout={onLayout}>
      ...
      <View style={styles.head}>
        ...
        <View style={{ alignItems: "flex-end" }}>
          ...
          <Text style={[styles.delta, { color: dcol }]}>
            {dv == null
              ? "기준 연도"
              : `${dv > 0 ? "▲" : dv < 0 ? "▼" : "■"} ${fmt(Math.abs(dv), yd.u)} 전년 대비`}
          </Text>
        </View>
      </View>

      <View style={styles.stats}>
        {[["11년 순위", `${rank}위 / ${YEARS.length}`], ["기간 최고", fmt(mx, yd.u)], ["기간 최저", fmt(mn, yd.u)]].map(
          ([k, val], idx) => (
      ...
      <Text style={styles.blockLbl}>{year}년, 이 지표는</Text>
