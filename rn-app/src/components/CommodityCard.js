...
export default function CommodityCard({ d }) {
  const { editValue } = useCommodities();
  ...
  return (
    <View style={styles.card}>
        ...
        <View style={{ alignItems: "flex-end" }}>
          ...
          {dv != null && (
            <Text style={[styles.delta, { color: dcol }]}>
              {dv > 0 ? "▲" : dv < 0 ? "▼" : "■"} {fmt(Math.abs(dv))} 직전 대비
            </Text>
          )}
        </View>
      </View>
      ...
      <View style={{ marginTop: 14 }}>
        <TrendChart points={points} kind={d.chart} color={col} />
      </View>
      ...
      <Accordion title="왜 이런 가격이 나왔나" defaultOpen color={col}>
        {d.why.map((p, i) => (
      ...
      <EditModal visible={editOpen} indicator={d} onClose={() => setEditOpen(false)} onSave={editValue} />
    </View>
  );
}
